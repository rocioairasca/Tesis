...
const getStats = async (req, res) => {
  try {
    ...
    let planningTotalQ = supabase.from('planning').select('id', { count: 'exact', head: true });
    planningTotalQ = applyPlanningFilters(planningTotalQ, { from: F, to: T, includeDisabled: incDisabled, includeCanceled: incCanceled });

    // completadas
    let planningCompletedQ = supabase.from('planning').select('id', { count: 'exact', head: true }).eq('status', 'completado');
    ...
    let planningCanceledQ = supabase.from('planning').select('id', { count: 'exact', head: true }).eq('status', 'cancelado');
